...
import {
  extraerDatosEmpleado, generarContratoDocx,
...
export default function ArmadoContratos({ plantillas }: Props) {
  const empresas = useMemo(
    () => [...new Set(plantillas.map(p => p.empresa))].sort((a, b) => a.localeCompare(b, 'es')),
    [plantillas],
  );

  const [empresaPref, setEmpresaPref] = useState('');
  const [areaPref, setAreaPref] = useState('');
  ...
  // empresa/área se derivan en render a partir de la preferencia del usuario y
  // de lo que realmente existe en el registro — así nunca apuntan a una
  // combinación borrada, sin necesidad de efectos que sincronicen estado.
  const empresa = empresas.includes(empresaPref) ? empresaPref : (empresas[0] ?? '');

  const areas = useMemo(
    () => [...new Set(plantillas.filter(p => p.empresa === empresa).map(p => p.area))]
      .sort((a, b) => a.localeCompare(b, 'es')),
    [plantillas, empresa],
  );

  const area = areas.includes(areaPref) ? areaPref : (areas[0] ?? '');

  const plantillaSel = useMemo(
    () => plantillas.find(p => p.empresa === empresa && p.area === area) ?? null,
    [plantillas, empresa, area],
  );

  const pendiente = !plantillaSel
    ? 'No hay una plantilla cargada para esta combinación. Cargala en la pestaña "Contratos originales".'
    : null;
  const placeholdersMal = plantillaSel != null && plantillaSel.placeholders !== PLACEHOLDERS_ESPERADOS;
  ...
  const handleArmar = useCallback(async () => {
    ...
    if (!plantillaSel) { setError('No hay una plantilla para esta empresa y área.'); return; }
    if (placeholdersMal) {
      setError(`La plantilla "${empresa} · ${area}" tiene ${plantillaSel.placeholders} campos "XXXXXXXX" en vez de ${PLACEHOLDERS_ESPERADOS}. Revisala en "Contratos originales".`);
      return;
    }
    ...
    try {
      ...
      const plantillaBuffer = await plantillaSel.docx.arrayBuffer();
      ...
      recordActivity('contratos', `${empresa} ${area} — ${nuevos.length} contrato${nuevos.length > 1 ? 's' : ''}`);
  ...
  }, [rawText, empresa, area, fecha, plantillaSel, placeholdersMal, renderizarSeleccion]);
      ...
      <div className="w-[400px] flex-shrink-0 border-r border-gray-200 bg-white p-5 overflow-y-auto print:hidden">
        <div className="mb-4">
          ...
          <p className="text-xs text-gray-500 leading-relaxed">
            Pegá los datos de uno o varios empleados. Empresa y área aplican a todo el lote.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div>
            ...
            <select
              value={empresa}
              onChange={e => setEmpresaPref(e.target.value)}
            ...
            >
              {empresas.length === 0 && <option value="">— sin plantillas —</option>}
          ...
          <div>
            ...
            <select
              value={area}
              onChange={e => setAreaPref(e.target.value)}
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#003DA5] focus:ring-1 focus:ring-[#003DA5]"
            >
              {areas.length === 0 && <option value="">—</option>}
        ...
        {placeholdersMal && (
          <div className="mb-4 flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2.5 text-xs text-amber-700">
            <AlertTriangle size={14} className="flex-shrink-0 mt-0.5" />
            <span>
              Esta plantilla tiene {plantillaSel?.placeholders} campos <code>XXXXXXXX</code> en vez de {PLACEHOLDERS_ESPERADOS}.
              Corregila en <b>Contratos originales</b> antes de armar.
            </span>
          </div>
        )}
        ...
        <button
          onClick={handleArmar}
          disabled={generando || !plantillaSel}
